...
test('media: PATCH /v1/media/:id updates metadata', async () => {
  ...
  await requestJson(handler, 'POST', `/v1/media/${init.json.mediaId}/finalize`, {
    token: accessToken,
    body: {
...
test('media: DELETE /v1/media/:id deletes media', async () => {
  ...
  const deleted = await requestJson(handler, 'DELETE', `/v1/media/${init.json.mediaId}`, { token: accessToken });
  assert.equal(deleted.res.status, 200);
  assert.equal(deleted.json.deleted, true);

  const notFound = await requestJson(handler, 'GET', `/v1/media/${init.json.mediaId}`, { token: accessToken });
...
test('media: GET /blob/:path* returns binary content with correct content-type', async () => {
  ...
  const media = await requestJson(handler, 'GET', `/v1/media/${init.json.mediaId}`, { token: accessToken });
  assert.equal(media.res.status, 200);
  const blobUrl = media.json.media.url;
  assert.ok(blobUrl);
...
test('media: POST /media/:id/finalize content-type fallback chain', async () => {
  ...
  assert.equal(finalized.json.media.mimeType, 'image/png');
});
