...
### Outstanding Items and Tracking
These items are deferred follow-up work and do not block this pivot PR merge.

- [ ] Phase D CSS pruning (`#TBD`): remove stale `.edit-post-*` selectors not referenced by `EditorSurface` or `Canvas` and prove no behavior regression.
- [ ] Phase E deterministic tie-break tests (`#TBD`): add equal-timestamp ordering tests for wp-core list endpoints with id tie-break validation.
- [ ] Phase F editor interaction e2e (`#TBD`): cover placeholder behavior, inserter repeatability, selection stability, and post/page navigation.
- [ ] Phase F theming isolation e2e (`#TBD`): verify no admin token bleed into content canvas during live theme updates.
- [ ] Phase F bootstrap shape assertions (`#TBD`): verify `/wp/v2/settings`, `/wp/v2/themes`, `/wp/v2/types`, and entity preload contracts.
...
- Verification: run grep checks for `initializeEditor` and assert no active canvas/runtime path references remain.

2. Gutenberg version drift
- Risk: accidental floating range upgrades
- Mitigation: exact pins + matrix check script in CI
- Verification: enforce `scripts/check-gutenberg-version-matrix.js` in CI.
...
- Verification: add selector audit plus theme shell parity checks in CI.
- Temporary posture: treat this as elevated risk until the deferred Phase D and Phase F theming tests land.
...
- Verification: unit tests for tie-break ordering and bootstrap payload shape assertions.

5. Production runtime failures
- Risk: unforeseen runtime regressions after switching ownership to `EditorSurface`.
- Mitigation: keep rollback branch path documented and ready (`Canvas` fallback path remains available for emergency hotfix).
- Verification: monitor editor initialization failures and runtime exceptions after rollout.

6. Observability gaps
- Risk: bootstrap failures or runtime errors go unnoticed in production.
- Mitigation: instrument bootstrap start/success/failure and uncaught runtime errors.
- Verification: set alerts on bootstrap failure rate and error-rate spikes.

7. Performance overhead
- Risk: per-instance registries/hot-swap behavior increases startup or interaction latency.
- Mitigation: capture baseline startup and interaction timing before/after rollout.
- Verification: compare metrics and fail follow-up gate on unacceptable regressions.

8. Content compatibility drift
- Risk: existing authored content exercises unsupported edge cases post-pivot.
- Mitigation: extend compatibility matrix with representative legacy samples.
- Verification: regression runs against saved fixtures before production rollout.

## Operational Deployment Plan

### Rollback Strategy
- Keep a rollback-ready patch path that reverts active runtime ownership to prior `Canvas` path if critical incidents appear.
- Trigger rollback on sustained bootstrap failures or editor crash spikes above baseline.

### Monitoring and Telemetry
- Track bootstrap lifecycle (`start`, `success`, `failure`) and runtime exceptions.
- Add dashboard slices for editor initialization success rate and median bootstrap latency.

### Performance Baseline
- Compare editor startup time and interaction latency before/after pivot using the same authoring scenario set.
- Treat regressions above agreed threshold as release blockers for follow-up phases.

### User-Facing Expectations
- No intended UX change; behavior should be more stable with same authoring model.
- If incidents surface, communicate rollback status and follow-up remediation.

### Success Metrics
- Reduced editor bootstrap failures.
- Reduced “inserter/selection stuck” incidents.
- Stable placeholder and block interaction behavior across refresh and document switches.
...
Notes:
- Items listed in "Outstanding Items and Tracking" are intentionally deferred follow-up work and remain required for full reliability closure.
