---
title: Content Model Draft (Historical)
---

# Content Model Draft (Historical)

> Note: Platform-level direction from this proposal is superseded by `docs/internal/edgepress-content-extensibility-spec.md` (2026-02-11). This file remains as a historical content-model draft.

This document captures a historical content-model draft that informed the canonical platform spec.

## Divergences from canonical platform spec

This document is intentionally non-canonical. Where it differs from the current platform source of truth (`/internal/edgepress-content-extensibility-spec`), follow the platform spec.

- Canonical authority: this draft is historical, not normative.
- Field strictness: this draft hard-commits to rejecting unknown `document.fields` keys; platform spec keeps this as a platform-level contract decision.
- Uniqueness scope: this draft states site/workspace-scoped uniqueness as if current; platform spec treats parts of this as target contract with implementation gaps.
- Slug normalization: this draft includes more specific algorithm detail than the current canonical spec contract.
- Revision snapshot policy: this draft states revisions do not snapshot `raw`; platform spec does not hard-require that policy.
...
All JSON snippets below are illustrative examples, not normative API/storage contracts.
