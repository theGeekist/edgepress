...
### Preview Embed Sanitization

Preview HTML generated by `/v1/preview/:documentId` applies a defensive embed policy before returning HTML:

- script tags are stripped
- inline event-handler attributes are stripped (for example `onclick=...`)
- `javascript:` URLs are stripped
- iframes are allowed only for known providers and same-origin/local embed paths
- other embed markup is removed/replaced

Preview responses also include a strict CSP header to block script execution.
...
const REQUIRED_WP_PROFILE = '1.1.0';
```
