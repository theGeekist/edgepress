...
test('media: POST /v1/media mirrors /v1/media/init and requires auth', async () => {
  const platform = createInMemoryPlatform();
  const { handler, accessToken } = await authAsAdmin(platform);

  const authed = await requestJson(handler, 'POST', '/v1/media', { token: accessToken, body: {} });
  assert.equal(authed.res.status, 201);
  assert.ok(authed.json.mediaId);
  assert.ok(authed.json.uploadToken);

  const unauthed = await requestJson(handler, 'POST', '/v1/media', { body: {} });
  assert.equal(unauthed.res.status, 401);
});

test('media routes: all protected handlers return auth envelopes when unauthorized', async () => {
  const platform = createInMemoryPlatform();
  const { handler } = await authAsAdmin(platform);

  const checks = [
    ['POST', '/v1/media/init', { body: {} }],
    ['POST', '/v1/media/med_x/finalize', { body: {} }],
    ['GET', '/blob/path/to/blob.jpg', {}],
    ['GET', '/v1/media', {}],
    ['GET', '/v1/media/med_x', {}],
    ['PATCH', '/v1/media/med_x', { body: {} }],
    ['DELETE', '/v1/media/med_x', {}]
  ];

  for (const [method, path, options] of checks) {
    const response = await requestJson(handler, method, path, options);
    assert.equal(response.res.status, 401, `${method} ${path} expected 401`);
  }

  const uploadRes = await handler(new Request('http://test.local/uploads/med_x', {
    method: 'PUT',
