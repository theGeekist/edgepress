...
test('resolveHooks warns once for partial registry and doAction logs sync hook errors', () => {
  ...
  const resolvedA = resolveHooks({
    hooks: partial,
    runtime: { log: (...args) => logs.push(args) }
  });
  const resolvedB = resolveHooks({
    hooks: partial,
    runtime: { log: (...args) => logs.push(args) }
  });

  assert.notEqual(resolvedA, partial);
  assert.equal(resolvedA, resolvedB);
  assert.equal(logs.length, 1);
});

test('app hooks applyFilters passthrough and doAction error logging branches', () => {
  const payload = { ok: true };
  assert.equal(applyFilters(null, 'x', payload), payload);
  assert.equal(applyFilters({}, 'x', payload), payload);
  assert.deepEqual(
    applyFilters({ applyFilters: (_name, value) => ({ ...value, touched: true }) }, 'x', payload),
  ...
  assert.throws(() => {
    doAction(
      ...
      { doAction() { throw new Error('hook boom'); } },
      'x',
      payload
    );
  }, /hook boom/);
