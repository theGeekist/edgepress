...
test('content-model: POST /v1/terms generates id when missing and returns domain errors', async () => {
  const platform = createInMemoryPlatform();
  platform.runtime.uuid = () => 'generated123';
  const { handler, accessToken } = await authAsAdmin(platform);

  await requestJson(handler, 'PUT', '/v1/taxonomies/topic', {
    token: accessToken,
    body: { label: 'Topics', hierarchical: false, objectTypes: ['post'] }
  });

  const created = await requestJson(handler, 'POST', '/v1/terms', {
    token: accessToken,
    body: { taxonomySlug: 'topic', name: 'Edge', slug: 'edge' }
  });
  assert.equal(created.res.status, 200);
  assert.equal(created.json.term.id, 'term_generated123');

  const invalid = await requestJson(handler, 'POST', '/v1/terms', {
    token: accessToken,
    body: { taxonomySlug: '', name: '', slug: '' }
  });
  assert.equal(invalid.res.status, 400);
  assert.equal(invalid.json.error.code, 'TERM_INVALID_TAXONOMY');
});

test('content-model routes: authz and invalid-json branches are normalized', async () => {
  const platform = createInMemoryPlatform();
  const { handler, accessToken } = await authAsAdmin(platform);

  const unauthorized = await Promise.all([
    requestJson(handler, 'GET', '/v1/content-types'),
    requestJson(handler, 'PUT', '/v1/content-types/article', { body: {} }),
    requestJson(handler, 'GET', '/v1/taxonomies'),
    requestJson(handler, 'PUT', '/v1/taxonomies/topic', { body: {} }),
    requestJson(handler, 'GET', '/v1/terms'),
    requestJson(handler, 'PUT', '/v1/terms/term_x', { body: {} }),
    requestJson(handler, 'POST', '/v1/terms', { body: {} })
  ]);
