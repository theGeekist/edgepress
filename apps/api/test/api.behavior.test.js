...
test('forms endpoint rejects invalid json body and preserves typed errors from requestContext', async () => {
  const platform = createInMemoryPlatform();
  const { handler } = await authAsAdmin(platform);

  const invalidJsonRes = await handler(new Request('http://test.local/v1/forms/contact/submit', {
    method: 'POST',
  ...
  const typedErr = await requestJson(handler, 'POST', '/v1/forms/contact/submit', {
    headers: { 'x-ip-hash': 'ip_custom', 'x-ua-hash': 'ua_custom' },
    body: { payload: { hello: 'world' } }
  });
  assert.equal(typedErr.res.status, 418);
  assert.equal(typedErr.json.error.code, 'TEAPOT');
});
