...
test('registry register normalizes defaults and de-duplicates arrays', () => {
  const imports = createImportTransformRegistry();
  const entry = imports.register({
    id: 'x',
    wpBlockNames: ['core/p', 'core/p', ''],
    toCanonical: () => ({ blockKind: 'ep/x' })
  });
  assert.deepEqual(entry.wpBlockNames, ['core/p']);
  assert.equal(typeof entry.canHandle, 'function');
  assert.deepEqual(imports.getById('missing'), null);

  const renderers = createRendererRegistry();
  const renderer = renderers.register({
    id: 'r',
    blockKinds: ['ep/p', 'ep/p', ''],
    targets: ['preview', 'preview', ''],
    render: () => 'ok'
  });
  assert.deepEqual(renderer.blockKinds, ['ep/p']);
  assert.deepEqual(renderer.targets, ['preview']);
  assert.deepEqual(renderers.getById('missing'), null);
});
