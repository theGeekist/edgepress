...
- `Increment complete`: replaced bespoke lifecycle hooks with canonical `@wordpress/hooks` semantics (`addAction`, `doAction`, `addFilter`, `applyFilters`) and added server-side JS bootstrap registration in composition roots (`apps/api/src/hooks-bootstrap.js`, `apps/api/src/server.js`, `apps/api/src/worker.js`).
